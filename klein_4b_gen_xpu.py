...
class LocalQwen3Embedder(nn.Module):
    ...
    def __init__(self, device="cpu"):
        ...
        if not os.path.isdir(TEXT_ENCODER_DIR) or not os.path.isdir(TOKENIZER_DIR):
            print("\n>> Local text encoder/tokenizer folder missing. Downloading from rootlocalghost/FLUX.2-klein-4B-FP16...")
            from huggingface_hub import snapshot_download
            snapshot_download(
                repo_id="rootlocalghost/FLUX.2-klein-4B-FP16",
                local_dir=MODEL_DIR,
...
# (legacy per-call generate_image removed; batch flow below)
...
if __name__ == "__main__":
    ...
    def generate_image(prompt: str, width: int = 1024, height: int = 1024, seed: int = 42, num_steps: int = 4, output_path: str = None):
        ...
        output_dir = os.path.join(SCRIPT_DIR, 'output/gen')
