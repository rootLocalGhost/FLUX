...
if not os.path.exists(TRANSFORMER_PATH):
    ...
    print(f">> Syncing with repository: rootlocalghost/FLUX.2-klein-4B-FP8")
    snapshot_download(
        repo_id="rootlocalghost/FLUX.2-klein-4B-FP8",
        local_dir=MODEL_DIR,
...
class LocalQwen3Embedder(nn.Module):
    ...
    def __init__(self, device="cpu"):
        ...
        if not os.path.isdir(TEXT_ENCODER_DIR) or not os.path.isdir(TOKENIZER_DIR):
            print("\n>> Local text encoder/tokenizer folder missing. Downloading from rootlocalghost/FLUX.2-klein-4B-FP8...")
            from huggingface_hub import snapshot_download
            snapshot_download(
                repo_id="rootlocalghost/FLUX.2-klein-4B-FP8",
                local_dir=MODEL_DIR,
